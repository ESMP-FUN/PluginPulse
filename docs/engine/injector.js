 ...
 *   opts   : { modrinth, github, githubToken, hangar, jenkins, jenkinsArtifact,
 *              permission, commandRoot, mode, contact, track,
 *              checkIntervalHours, holdNewUpdatesHours, hotReload, upgrade }
...
(function (root) {
  ...
  function buildPulseYaml(o) {
    ...
    // Quoted: a ${ENV_VAR} reference must not be read as a YAML alias/anchor.
    if (o.githubToken) lines.push('github-token: "' + o.githubToken.replace(/"/g, '\\"') + '"');
    ...
    // The hot-reload module travels in the payload, so this key is honoured in a
    // browser-injected jar; the engine still refuses at runtime when unsafe.
    if (o.hotReload) lines.push('hot-reload: true');
    return lines.join('\n') + '\n';
  }
